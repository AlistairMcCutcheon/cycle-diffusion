...
class Evaluator(object):
    def __init__(self, args, meta_args):
        ...
        self.ref_transform = transforms.Compose(
            [
                transforms.Resize(64, interpolation=INTERPOLATION),  # 512 -> 256
                transforms.ToTensor(),
            ]
        )
    ...
    def evaluate(self, images, model, weighted_loss, losses, data, split):
        ...
        for original_img, img in tqdm(images):
            ...
            img = (img - torch.amin(img)) / (torch.amax(img) - torch.amin(img))
            img = img.clamp(0, 1)
            ...
            l2 += torch.sqrt(((img - original_img) ** 2).sum(2).sum(1).sum(0)).item()
